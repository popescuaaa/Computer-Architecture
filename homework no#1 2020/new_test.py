...
def main():
    ...
    for k, products_dict in market_config['products'].items():
        params = {
            k: products_dict[k]
            for k in products_dict.keys() if k != 'product_type'
        }
    ...
    for producer in market_config['producers']:
        producer['products'] = [
            (products[i], quantity, sleep_time)
            for i, quantity, sleep_time in producer['products']
        ]
    ...
    producers = [
        Producer(**p_market_config, marketplace=marketplace, daemon=True)
    ...
    consumers = [
        Consumer(**c_market_config, marketplace=marketplace)
